{{-- <nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="url('redirect')" :active="request()->routeIs('dashboard')">
                {{ __('Dashboard') }}
            ...
            <div class="flex w-full">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex justify-between items-center w-full">
                    <div class="space-x-8 items-center">
                        <x-nav-link :href="url('/redirect')" :active="request()->routeIs('products.list')">
                            {{ __('Products') }}
                        </x-nav-link>
                        <x-nav-link :href="url('/userorders')" :active="request()->routeIs('dashboard')">
                            {{ __('Orders') }}
                        </x-nav-link>
                        <x-nav-link :href="url('/wishlist')" :active="request()->routeIs('wishlist')">
                            {{ __('Wishlist') }}
                        </x-nav-link>
                    </div>
                    <form action="{{ url('user-search') }}" method="post" class="flex items-center">
                        @csrf
                        <div class="relative rounded-md shadow-sm">
                            <input type="search" id="search" name="search" class="form-input py-2 pr-10 pl-4 sm:text-sm rounded-l-md focus:ring focus:border-blue-300" placeholder="Search now">
                            {{-- <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <svg class="w-5 h-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M13.293 14.707a1 1 0 0 1-1.414 0l-3-3a1 1 0 1 1 1.414-1.414L12 11.586l2.293-2.293a1 1 0 1 1 1.414 1.414l-3 3z" clip-rule="evenodd" />
                                </svg>
                            </div> --}}
                        </div>
                        <button type="submit" class="ml-4 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md">Search</button>
                    </form>
